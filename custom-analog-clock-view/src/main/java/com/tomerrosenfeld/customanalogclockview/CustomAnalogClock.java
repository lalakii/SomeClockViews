...
public class CustomAnalogClock extends View {
  ...
  public CustomAnalogClock(Context context, boolean defaultWatchFace) {
    super(context);
    if (defaultWatchFace) init(context);
  }
  ...
  public void withScale(float scale) {
    if (scale <= 0) throw new IllegalArgumentException("Scale must be bigger than 0");
  ...
  public void init(
    ...
    assert hHand != null;
    if (alpha > 0) hHand.setAlpha(alpha);
  ...
  public CustomAnalogClock init(
    ...
    CustomAnalogClock.hourOnTop = hourOnTop;
    setFace(watchFace);
    if (alpha > 0) hourHand.setAlpha(alpha);
    mCalendar = Calendar.getInstance();
