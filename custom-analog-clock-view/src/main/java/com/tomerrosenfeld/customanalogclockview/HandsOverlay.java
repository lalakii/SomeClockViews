...
public class HandsOverlay implements DialOverlay {
  ...
  public void onDraw(
    ...
    canvas.save();
    if (!CustomAnalogClock.hourOnTop) drawHours(canvas, cX, cY, calendar, sizeChanged);
    else drawMinutes(canvas, cX, cY, calendar, sizeChanged);
    canvas.restore();

    canvas.save();
    if (!CustomAnalogClock.hourOnTop) drawMinutes(canvas, cX, cY, calendar, sizeChanged);
    else drawHours(canvas, cX, cY, calendar, sizeChanged);
    canvas.restore();
  }
